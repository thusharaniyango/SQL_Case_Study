...
SELECT COUNT(membercost) AS nocost_count  FROM `Facilities` WHERE membercost > 0
...
FROM Facilities
WHERE membercost < (.02 * monthlymaintenance);
...
SELECT subqry.court, CONCAT( subqry.fname,  ' ', subqry.sname ) AS name
FROM (
SELECT Facilities.name AS court, Members.firstname AS fname, Members.surname AS sname
FROM Bookings
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
AND Facilities.name LIKE  'Tennis Court%'
INNER JOIN Members ON Bookings.memid = Members.memid
) subqry
GROUP BY subqry.court, subqry.fname, subqry.sname
ORDER BY name
